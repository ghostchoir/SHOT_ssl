...
def k_means(X, n_clusters, init='similar_cut', sparsity=None, max_iter=10,
            ...
            epsilon=5.0, minimum_df_factor=0.01, sample_weight=None):
...
def kmeans_single(X, n_clusters, max_iter=10, init='similar_cult', sparsity=None,
    ...
    centers, labels, inertia, n_iter_ = _kmeans_single_banilla(
        ...
        radius, epsilon, minimum_df_factor, sample_weight=sample_weight)
...
def _kmeans_single_banilla(X, sparsity, n_clusters, centers, max_iter,
                           ...
                           radius, epsilon, minimum_df_factor, sample_weight):
    ...
    for n_iter_ in range(1, max_iter + 1):
        ...
        centers = _update(X, labels, distances, n_clusters, sample_weight)

        if sample_weight is None:
            inertia = distances.sum()
        else:
            inertia = np.inner(distances, sample_weight)
...
def _update(X, labels, distances, n_clusters, sample_weight=None):
