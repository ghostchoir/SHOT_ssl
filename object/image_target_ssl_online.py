    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 and (args.cls_par > 0 or args.ssl_task in ['supcon', 'ls_supcon', 'crsc']):
            ...
            if centroid is None or args.recompute_centroid:
                mem_label, mem_conf, centroid, labelset = obtain_label(dset_loaders['pl'], netF, netH, netB, netC, args)
...
def compute_pl(args, feature, centroid, labelset):
    ...
    pl = torch.from_numpy(pl).cuda()

    return pl


def update_centroid(args, feature, centroid, output, labelset):
    feature = feature.detach().float().cpu().numpy()
